...
def analyze_block(block_range):
    ...
    try:
        events = list()
        events += get_events(w3, client_version, {"fromBlock": block_range[0], "toBlock": block_range[1], "topics": [UNISWAP_V2]},  OPTIMISM_PROVIDER, "optimism")
        events += get_events(w3, client_version, {"fromBlock": block_range[0], "toBlock": block_range[1], "topics": [UNISWAP_V3]},  OPTIMISM_PROVIDER, "optimism")
    ...
    for block_number in events_per_block:
        status = mongo_connection["cross_chain_arbitrage"]["detected_single_chain_arbitrage_optimism_status"].find_one({"block_number": block_number})
        ...
        one_eth_to_usd_price = decimal.Decimal(float(get_price_from_timestamp("optimism", WETH, block["timestamp"], ALLIUM_API_KEY, cache)))
                    ...
                    if swaps[tx_index][0]["in_amount"]  <= swaps[tx_index][-1]["out_amount"] and \
                       ...
                       (swaps[tx_index][0]["in_token"]  == swaps[tx_index][-1]["out_token"] or (swaps[tx_index][0]["in_token"] in [ETH, WETH] and swaps[tx_index][-1]["out_token"] in [ETH, WETH])):
                                ...
                                for swap in intermediary_swaps:
                                    if not swap["in_token"] in intermediary_gains:
                                        ...
                                        if swap["in_token"] == ETH:
                                            ...
                                            try:
                                                one_token_to_usd_price = decimal.Decimal(float(get_price_from_timestamp("optimism", swap["in_token"], block["timestamp"], ALLIUM_API_KEY, cache)))
                                            except:
                                                one_token_to_usd_price = None
                                    ...
                                    if not swap["out_token"] in intermediary_gains:
                                        ...
                                        if swap["out_token"] == ETH:
                                            ...
                                            try:
                                                one_token_to_usd_price = decimal.Decimal(float(get_price_from_timestamp("optimism", swap["out_token"], block["timestamp"], ALLIUM_API_KEY, cache)))
                                            except:
                                                one_token_to_usd_price = None
                        ...
                        if valid:
                            ...
                            collection = mongo_connection["cross_chain_arbitrage"]["detected_single_chain_arbitrage_optimism"]
                            try:
                                if DEBUG_MODE:
                                    import pprint
        ...
        collection = mongo_connection["cross_chain_arbitrage"]["detected_single_chain_arbitrage_optimism_status"]
        try:
            if not DEBUG_MODE:
...
def main():
    ...
    for block in range(block_range_start, block_range_end+1):
        ...
        if counter == BLOCK_RANGE or block == block_range_end:
            block_range.append(block)
            count = mongo_connection["cross_chain_arbitrage"]["detected_single_chain_arbitrage_optimism_status"].count_documents({"block_number": {"$gte": block_range[0], "$lte" : block_range[1]}})
