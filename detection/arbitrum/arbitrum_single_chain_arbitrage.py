...
def analyze_block(block_range):
    ...
    for block_number in events_per_block:
        status = mongo_connection["cross_chain_arbitrage"]["detected_single_chain_arbitrage_arbitrum_status"].find_one({"block_number": block_number})
        ...
        try:
            for event in events:
                ...
                if event["topics"][0].lower() in UNISWAP_V2.lower():
                    ...
                    _amount0In  = int(event["data"].hex()[0:64], 16)
                    _amount1In  = int(event["data"].hex()[64:128], 16)
                    _amount0Out = int(event["data"].hex()[128:192], 16)
                    _amount1Out = int(event["data"].hex()[192:256], 16)
                ...
                # Search for Uniswap V3 swaps
                    ...
                    if not event["transactionIndex"] in swaps:
                        swaps[event["transactionIndex"]] = list()
                    _amount0 = to_signed_256(int(event["data"].hex()[0:64], 16))
                    _amount1 = to_signed_256(int(event["data"].hex()[64:128], 16))
                    exchange_contract = w3.eth.contract(address=event["address"], abi=[
                        {"inputs":[],"name":"token0","outputs":[{"internalType":"address","name":"","type":"address"}],"stateMutability":"view","type":"function"},
                        {"inputs":[],"name":"token1","outputs":[{"internalType":"address","name":"","type":"address"}],"stateMutability":"view","type":"function"}
                    ])
        ...
        one_eth_to_usd_price = decimal.Decimal(float(get_price_from_timestamp("arbitrum", WETH, block["timestamp"], ALLIUM_API_KEY, cache)))
                    ...
                    if swaps[tx_index][0]["in_amount"]  <= swaps[tx_index][-1]["out_amount"] and \
                       ...
                       (swaps[tx_index][0]["in_token"]  == swaps[tx_index][-1]["out_token"] or (swaps[tx_index][0]["in_token"] in [ETH, WETH] and swaps[tx_index][-1]["out_token"] in [ETH, WETH])):
                                ...
                                for swap in intermediary_swaps:
                                    if not swap["in_token"] in intermediary_gains:
                                        ...
                                        if swap["in_token"] == ETH:
                                            ...
                                            try:
                                                one_token_to_usd_price = decimal.Decimal(float(get_price_from_timestamp("arbitrum", swap["in_token"], block["timestamp"], ALLIUM_API_KEY, cache)))
                                            except:
                                                one_token_to_usd_price = None
                                    ...
                                    if not swap["out_token"] in intermediary_gains:
                                        ...
                                        if swap["out_token"] == ETH:
                                            ...
                                            try:
                                                one_token_to_usd_price = decimal.Decimal(float(get_price_from_timestamp("arbitrum", swap["out_token"], block["timestamp"], ALLIUM_API_KEY, cache)))
                                            except:
                                                one_token_to_usd_price = None
                        ...
                        if valid:
                            ...
                            tx_cost = Web3.from_wei(receipt["gasUsed"] * tx["gasPrice"], "ether")
                            if tx_cost != 0:
                            ...
                            collection = mongo_connection["cross_chain_arbitrage"]["detected_single_chain_arbitrage_arbitrum"]
                            try:
                                if DEBUG_MODE:
                                    import pprint
        ...
        collection = mongo_connection["cross_chain_arbitrage"]["detected_single_chain_arbitrage_arbitrum_status"]
        try:
            if not DEBUG_MODE:
...
def main():
    ...
    for block in range(block_range_start, block_range_end+1):
        ...
        if counter == BLOCK_RANGE or block == block_range_end:
            block_range.append(block)
            count = mongo_connection["cross_chain_arbitrage"]["detected_single_chain_arbitrage_arbitrum_status"].count_documents({"block_number": {"$gte": block_range[0], "$lte" : block_range[1]}})
